...
def test_quantum_gate():
    ...
    for op_str in good_inputs:
        ...
        assert np.allclose(gate.construct_gate(), input_dict[op_str])
        assert np.allclose(gate.dagger(), np.transpose(np.conj(input_dict[op_str])))
        if op_str == 'T':
            assert not np.allclose(gate.dagger(), gate.construct_gate()) # nonhermitian op
        else:
            assert np.allclose(gate.dagger(), gate.construct_gate()) # hermitian ops
        assert gate.dim() == (2,2)
...
def test_nquantum_gate():
    ...
    for op_str in good_inputs:
        ...
        assert np.allclose(gate.construct_gate(), np.kron(input_dict[op_str], identity))
        gate = gate_class.nQuantumGate(op_str, nqb, 2)
        assert np.allclose(gate.construct_gate(), np.kron(identity, input_dict[op_str]))
    ...
    for pauli_product in product(list(good_inputs), repeat=2):
        ...
        assert np.allclose(gate.construct_gate(), np.kron(input_dict[pauli_product[0]], input_dict[pauli_product[1]]))
        assert gate.dim() == (2**nqb, 2**nqb)
    ...
    gate.construct_gate()  # call construction of true array
...
def test_control_gate():
    ...
    for input in good_inputs:
        ...
        assert np.allclose(C12.construct_gate(), np.block([[identity, np.zeros((2,2))],[np.zeros((2,2)), input_dict[input]]]))
    CX21 = gate_class.ControlGate('Z', nqb, cont = 2, targ = 1)
    check = np.block([[identity, np.zeros((2,2))],[np.zeros((2,2)), Z]])
    assert np.allclose(CX21.construct_gate(), check)
